...
export function TourInfoBoxes({ tour }: { tour: Tour }) {
  const boxes = [
    ...
    { icon: <UsersIcon />, label: 'Group size', value: `Up to ${tour.groupSizeMax}` },
    { icon: <TagIcon />, label: 'Trip type', value: tour.categoryInfo?.name ?? 'Package' },
    {
      icon: <GlobeIcon />,
      label: tour.departsFrom ? 'Starts from' : 'Countries',
